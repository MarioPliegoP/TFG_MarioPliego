...
plot(out.ai.signals.values,out.do.signals.values,'b--','LineWidth',line);
hold("on")
plot(AI,DO,'r--','LineWidth',line)
plot(vectorx,vectory,'p-','LineWidth',1)
plot(final,1:2^B-1,'.y','MarkerSize',12)
plot(inicio,0:2^B-2,'.y','MarkerSize',12)
legend('Simulated Transfer Characteristic','Ideal Transfer Characteristic','Correction: Linking points','Location','southeast')
xlabel('AI')
ylabel('DO')
title('ADC Transfer Characteristic')
hold("off")
...
plot(AI,DO,'r--','LineWidth',line)
hold("on")
plot(vectorx2,vectory2,'b--','LineWidth',line)
legend('Theoretical Transfer Characteristic','Gain&Offset Corrected Characteristic','Location','southeast')
xlabel('AI')
ylabel('DO')
hold("off")
...
plot(0:2^B-1,[0,INL])
title(sprintf('INL(k)'))
legend(sprintf('|INL|_{max}= (%3.2f ± %3.2f) LSB',INL_max, INL_tolerance))
xlabel('Code')
ylabel('INL(k)')
grid('on')
